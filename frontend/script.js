...
const completeCode = document.getElementById('completeCode').querySelector('code');
...
async function generatePage(tema) {
  ...
  try {
    ...
    showError(err.message || 'Erro ao conectar com o servidor. Verifique se o backend está rodando na porta 3001.');
  } finally {
    setLoading(false);
  }
}
...
function displayResult(tema, data) {
  ...
  urlSlug.textContent = tema.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
...
document.querySelectorAll('.copy-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    if (!text) { showToast('⚠️ Nada para copiar', true); return; }
